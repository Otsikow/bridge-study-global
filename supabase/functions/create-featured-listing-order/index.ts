...
const PLAN_CONFIGS: Record<PlanCode, PlanConfig> = {
  spotlight_30: {
    ...
    defaultPriority: 8,
    features: [
      "Featured placement for one month",
      "Inclusion in weekly student newsletter",
      "Performance summary report at the end of the cycle",
    ],
  },
  spotlight_90: {
    ...
    defaultPriority: 5,
    features: [
      "Quarterly featured placement",
      "Newsletter inclusion + homepage hero rotation",
      "Bi-weekly campaign insights",
    ],
  },
  spotlight_180: {
    ...
    defaultPriority: 2,
    features: [
      "Half-year strategic spotlight",
      "Premium hero rotation with conversion copy refresh",
      "Monthly campaign review with Partner Success team",
    ],
  },
};
...
function sanitizePriority(value: unknown): number | null {
  ...
  const clamped = Math.max(0, Math.min(99, Math.round(parsed)));
  return clamped;
}
...
type RpcHasRoleResult = boolean | null;
...
serve(async (req) => {
  ...
  try {
    ...
    if (!universityId) {
      return new Response(JSON.stringify({ error: "universityId is required" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (!planCode || !(planCode in PLAN_CONFIGS)) {
      return new Response(JSON.stringify({ error: "planCode is invalid" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (!summary) {
      return new Response(JSON.stringify({ error: "A summary headline is required" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (profileError || !profile) {
      ...
      return new Response(JSON.stringify({ error: "Unable to resolve profile" }), {
        status: 403,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (partnerRoleError || adminRoleError) {
      console.error("Role resolution failed", partnerRoleError ?? adminRoleError);
    ...
    const { data: university, error: universityError } = await supabase
      .from("universities")
      .select(
        "id, tenant_id, featured_listing_current_order_id, featured_listing_status, featured_priority, featured_summary, featured_highlight, featured_image_url"
      )
      .eq("id", universityId)
      .single();
    ...
    if (!isAdminOrStaff && university.tenant_id !== profile.tenant_id) {
      return new Response(JSON.stringify({ error: "You can only manage spotlighting for your institution" }), {
        status: 403,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    // If a previous order is active, expire it gracefully
    if (university.featured_listing_current_order_id) {
      await supabase
        ...
        .eq("id", university.featured_listing_current_order_id)
    ...
    if (orderError || !order) {
      ...
      return new Response(JSON.stringify({ error: "Unable to create featured listing order" }), {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    const { data: updatedUniversity, error: updateUniversityError } = await supabase
      ...
      .update({
        featured: true,
        featured_summary: summary,
        featured_highlight: highlight,
        featured_image_url: imageUrl,
        featured_priority: priority,
        featured_listing_status: "active",
        featured_listing_current_order_id: order.id,
        featured_listing_expires_at: expiresIso,
        featured_listing_last_paid_at: nowIso,
      })
      .eq("id", universityId)
      .select(
        "id, name, featured, featured_priority, featured_summary, featured_highlight, featured_image_url, featured_listing_status, featured_listing_expires_at, featured_listing_last_paid_at, featured_listing_current_order_id"
      )
      .single();

    if (updateUniversityError || !updatedUniversity) {
      console.error("University update failed", updateUniversityError);
      await supabase
        .from("featured_listing_orders")
        .update({ status: "pending" })
        .eq("id", order.id);

      return new Response(JSON.stringify({ error: "Unable to update university showcase state" }), {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
